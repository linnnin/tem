...
if __name__ == '__main__':
    ...
    #origin_QA = args[1]
    #key = args[2]
    client = ZhipuAI(api_key="e725cddeb96733573d6ad7f665137407.lNiXBXBRzumvSjgO")  # 填写你自己的APIKey

    while True:
        file = open("./target/generation_instruction.txt", 'a', encoding="UTF-8")

        # 打开 Excel 文件
        #workbook = load_workbook(origin_QA)
        workbook = load_workbook("./QA.xlsx")
        # 获取工作表
        sheet = workbook.active
        # 遍历行和列
        input_list = []
        output_list = []
        for row in sheet.iter_rows(min_row=2, max_row=120, values_only=True):
            input_list.append(list(row)[0])
            output_list.append(list(row)[1])
        print(input_list)
        print(output_list)

        with open("./target/generation_instruction.txt", 'r', encoding="UTF-8") as file2:
            generation_instructions_list = file2.readlines()

        if len(generation_instructions_list) >= 20000:
            break
        ...
        # 建立模版
        prompt = "你是一名汽车供应链领域的专家，下面是一些与汽车供应链相关的问题：\n"
        ...
        prompt += "请参考上面问题的内容和形式，对上述问题进行补充和拓展，再提出10个关于汽车供应链的比较深入的问题"
        print(prompt)
